<template>
	<view class="siyuan">
		...
		<u-alert-tips v-show="goodsInfo.content&&goodsInfo.content!='null'" class="u-margin-15" type="error" title="拒发原因"
		 :show-icon="true" :description="goodsInfo.content"></u-alert-tips>
		<u-alert-tips v-show="goodsInfo.revokeAppRemark && goodsInfo.revokeAppRemark.length>0" class="u-margin-15" type="warning"
		 title="撤销原因" :show-icon="true" :description="goodsInfo.revokeAppRemark"></u-alert-tips>
		<u-alert-tips v-show="goodsInfo.revokeContent && goodsInfo.revokeContent.length>0" class="u-margin-15" type="error"
		 title="驳回原因" :show-icon="true" :description="unRichHtml(goodsInfo.revokeContent)"></u-alert-tips>
...
<script>
	...
	export default {
		...
		onLoad(options) {
			this.getInfo(options.id, options.content)
		},
		methods: {
			/**
			 * 获取发货申请详请
			 * @param {type} id 主键
			 * @param {type} content 拒发原因
			 * @param {type} revokeContent 驳回原因
			 */
			getInfo(id, content) {
				uni.showLoading({
					title: '加载中'
				...
				getShipInfo({
					...
					if (res.result) {
						...
						this.goodsInfo.content = this.unRichHtml(content)
